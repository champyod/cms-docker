...
# Or edit config.toml [worker] directly, then regenerate .env:
#   WORKER_N=hostname:port entries are managed by the Admin Panel UI
#   or ./cms worker — not by hand-editing .env (generated file)
./cms config sync
...
- `.env` - Generated by `./cms config sync` — do not edit directly (unified env for all stacks)
