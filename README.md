...
| `.env` | Generated by `./cms config sync` — do not edit directly (unified env) |
| `.env.contest` | Optional per-contest overrides for the deploy scripts (ports, TLS, limits) — `CONTEST_ID` lives in `config.toml` |
