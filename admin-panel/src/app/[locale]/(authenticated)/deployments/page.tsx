...
export default function DeploymentsPage() {
  ...
  useEffect(() => {
    ...
    if (currentStr !== originalConfig) {
        analyzeRestartRequirements(['CONTESTS_DEPLOY_CONFIG']).then(res => {
  ...
  const handleSave = async (shouldRestart: boolean = false) => {
    ...
    const result = await updateEnvFile('.env.contest', { CONTESTS_DEPLOY_CONFIG: configStr });
  ...
  return (
    <div className="space-y-8 p-8">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-white mb-2">Contest Deployments</h1>
          <p className="text-neutral-400">Manage multiple contest instances running on different ports.</p>
        </div>
        <div className="flex gap-3">
            <button
                onClick={() => handleSave(false)}
                disabled={saving || JSON.stringify(config) === originalConfig}
        ...
        <div className="space-y-4">
            <div className="grid grid-cols-1 gap-3">
                {config.map((item, index) => (
                    <div key={index} className="flex items-center gap-4 bg-black/40 p-4 rounded-xl border border-white/5 group hover:border-indigo-500/30 transition-colors">
                        ...
                        <div className="flex-1 grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <div className="space-y-1">
                                <label className="text-[10px] font-bold text-neutral-500 uppercase tracking-widest">Domain (Traefik)</label>
                                <input 
                                    type="text" 
                                    value={item.domain || ''} 
                                    onChange={(e) => {
            ...
            <button 
                onClick={addItem}
                className="flex items-center gap-2 px-4 py-3 bg-white/5 hover:bg-white/10 text-white rounded-xl transition-all w-full justify-center border border-white/5"
            >
