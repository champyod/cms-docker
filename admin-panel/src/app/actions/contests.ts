...
function buildContestInsertDefaults(data: ContestData) {
...
async function insertContestRow(data: ContestData, d: ReturnType<typeof buildContestInsertDefaults>): Promise<void> {
...
async function executeContestUpdate(
  id: number,
  data: Partial<ContestData>,
  iv: ReturnType<typeof buildUpdateIntervalParams>
): Promise<void> {
  ...
  await prisma.$executeRaw`
    UPDATE contests SET
      ...
      start = COALESCE(${startDate}, start), stop = COALESCE(${stopDate}, stop),
      ...
      allow_questions = COALESCE(${data.allow_questions}, allow_questions), allow_user_tests = COALESCE(${data.allow_user_tests}, allow_user_tests),
...
export async function updateContest(id: number, data: Partial<ContestData>) {
  ...
  try {
    await executeContestUpdate(id, data, buildUpdateIntervalParams(data));
