...
export const INFRASTRUCTURE_ITEMS: NavItemDef[] = [
  ...
  { label: 'Groups', icon: ShieldCheck, buildHref: (locale) => `/${locale}/groups`, isVisible: (effective) => hasEffectivePermission(effective, 'group:list') },
  { label: 'Audit', icon: ScrollText, buildHref: (locale) => `/${locale}/audit`, isVisible: (effective) => hasEffectivePermission(effective, 'audit:read') },
