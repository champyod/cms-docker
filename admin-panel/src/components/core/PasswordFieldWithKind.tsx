...
export function PasswordKindSelector({
  ...
  return (
    <div
      className="inline-flex items-center gap-0.5 p-0.5 bg-black/60 border border-white/10 rounded-lg"
      role="group"
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <span className="text-[11px] uppercase tracking-wider text-neutral-500">Storage</span>
        ...
        {reveal?.state === 'plaintext' && (
          <button
            ...
            className="inline-flex items-center gap-1 px-2 py-1 text-[11px] font-medium bg-white/10 hover:bg-white/15 text-neutral-200 border border-white/10 rounded-lg transition-colors disabled:opacity-50"
          >
      ...
      {reveal?.state === 'plaintext' && isRevealedVisible && !reveal.loading && (
        <input
          ...
          className="w-full px-3 py-2 bg-black/50 border border-white/10 rounded-lg font-mono text-sm text-emerald-300 focus:outline-none"
        />
      )}
    </div>
  );
