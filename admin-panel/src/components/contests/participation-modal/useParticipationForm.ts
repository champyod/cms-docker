...
const INITIAL: ParticipationFormData = {
  team_id: null, hidden: false, unrestricted: false, extra_time_seconds: 0, delay_time_seconds: 0, ip: '', starting_time: '', password: '',
};
...
export function useParticipationForm(isOpen: boolean, participationId: number) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (data) {
        setFormData({ team_id: data.team_id, hidden: data.hidden, unrestricted: data.unrestricted, extra_time_seconds: data.extra_time_seconds, delay_time_seconds: data.delay_time_seconds, ip: data.ip_string, starting_time: data.starting_time, password: '' });
      }
    } catch {
