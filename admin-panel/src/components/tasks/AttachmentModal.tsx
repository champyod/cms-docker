          ...
          <form onSubmit={handleSubmit} className="p-4 space-y-4">
            <div>
              ...
              <div className="relative">
                ...
                <label htmlFor="attachment-file" className="flex items-center justify-center gap-2 w-full px-4 py-8 bg-black/30 border border-dashed border-white/20 rounded-lg text-neutral-400 cursor-pointer hover:bg-black/50 hover:border-purple-500/30 transition-colors">
            ...
            <div>
              <label className="block text-xs font-bold text-neutral-500 uppercase mb-2">Filename (as shown to contestants)</label>
              <input type="text" value={customFilename} onChange={(event) => setCustomFilename(event.target.value)} placeholder="e.g., sample_data.zip" className="w-full px-4 py-3 bg-black/50 border border-white/10 rounded-lg text-white focus:outline-none focus:border-indigo-500/50" />
            </div>
            ...
            <div className="flex gap-3 pt-4">
              ...
              <button type="submit" disabled={loading || !file} className="flex-1 px-4 py-3 bg-purple-600 hover:bg-purple-500 text-white rounded-lg transition-colors disabled:opacity-50 flex items-center justify-center gap-2">
                {loading ? (
                  <>
