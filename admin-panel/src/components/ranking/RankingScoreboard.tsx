...
function ScoreboardMobileCards({ rows }: { rows: RankRow[] }): React.JSX.Element[] {
  return rows.map((row) => (
    <MobileCard key={row.userId}>
      <MobileCardRow label="Rank" value={row.rank} />
      <MobileCardRow label="Name" value={`${row.firstName} ${row.lastName}`} />
      <MobileCardRow label="Team" value={row.team} />
      <MobileCardRow label="Solved" value={row.solved} />
      <MobileCardRow label="Total Score" value={formatScore(row.totalScore)} />
    </MobileCard>
  ));
}

function ScoreboardTable({ rows }: { rows: RankRow[] }): React.JSX.Element {
  return (
    <Table outerClassName="overflow-x-auto" className="w-max table-auto" mobileCards={<ScoreboardMobileCards rows={rows} />}>
      <TableHeader>
        <TableRow>
      ...
      <TableBody>
        {rows.map((row) => (
          <TableRow key={row.userId}>
            ...
            <TableCell className="text-right font-mono">{formatScore(row.totalScore)}</TableCell>
          </TableRow>
        ))}
      </TableBody>
    </Table>
...
export function RankingScoreboard({ rows, loadingSnapshot }: Props): React.JSX.Element {
  return (
    <Card>
      <Stack direction="col" gap={4}>
        ...
        {!loadingSnapshot && rows.length > 0 && <ScoreboardTable rows={rows} />}
      </Stack>
    </Card>
  );
}
