...
def build_isolate():
    ...
    subprocess.check_call(["make", "-C", "src/isolate", "isolate"])
...
def install_isolate():
    ...
    if not os.path.exists(os.path.join("src", "isolate", "isolate")):
    ...
    copyfile(os.path.join(".", "src", "isolate", "isolate"),
...
def install():
    ...
    path = os.path.join("src", "cmscontrib", "loaders", "polygon", "testlib.h")
