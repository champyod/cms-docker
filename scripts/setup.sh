...
if [ -f .env.core ]; then
    ...
    # Secrets detection
    DETECTED_AUTH_SECRET=$(grep "^AUTH_SECRET=" .env.admin 2>/dev/null | cut -d '=' -f2-)
    DETECTED_CONTEST_SECRET=$(grep "^SECRET_KEY=" .env.contest 2>/dev/null | cut -d '=' -f2-)
    DETECTED_CMS_SECRET=$(grep "^CMS_SECRET_KEY=" .env.core 2>/dev/null | cut -d '=' -f2-)
